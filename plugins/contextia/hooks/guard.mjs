...
// shell:true so Windows resolves the `contextia.cmd` shim that npm installs;
// args are fixed and the prompt goes via stdin, so there's nothing to inject.
const scan = spawnSync('contextia', ['scan', '--json'], { input: prompt, encoding: 'utf8', shell: true })
